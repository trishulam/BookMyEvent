                ...
                <template v-slot="{ tags, disabled, addTag, removeTag }">
                    <ul v-if="tags.length > 0" class="list-inline d-inline-block mb-2">
                        <li v-for="tag in tags" :key="tag" class="list-inline-item">
                            <b-form-tag @remove="removeTag(tag); changeres(tag)" :title="tag" :disabled="disabled" variant="info">{{ tag
                            }}</b-form-tag>
                        </li>
                    </ul>
...
export default {
    data() {
        return {
            options: ['Action','Horror','Animation','Drama','Science-Fiction','Thriller','Fantasy','War','Mystery'],
            search: '',
            value: [],
            result: []
        }
